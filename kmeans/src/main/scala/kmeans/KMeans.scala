...
class KMeans {
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    if(oldMeans.isEmpty){
      ...
      if( !(oldMeans.head.squareDistance(newMeans.head) < eta)){
        ...
        converged(eta)(oldMeans.tail, newMeans.tail)
      }
    }
    */
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    //*
    var updated: GenSeq[Point] = List()
      do{
        updated = update(classify(points, means), means)
      }while(converged(eta)(means, updated))
    updated
  /*/
...
class Point(val x: Double, val y: Double, val z: Double) {
  def values : List[Double] = List(x, y, z)
