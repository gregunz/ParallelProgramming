...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad = {
      if ( size > minimumSize ){
        ...
        (bodies :+ b).foldLeft(f)( _ insert _ )
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          if(quad.size / distance(quad.centerX, quad.centerY, x, y) < theta){ //cluster
            addForce(quad.mass, quad.massX, quad.massY)
          } else {
            List(nw, ne, sw, se).foreach( traverse(_) )
          }
      }
  ...
  def bounded(a: Float, min: Float, max: Float): Float = Math.min(Math.max(a, min), max)
  /*{
    if(a > max) max
    else if (a < min) min
    else a
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x = bounded(b.x - boundaries.minX, 0, boundaries.width) / sectorSize
      val y = bounded(b.y - boundaries.minY, 0, boundaries.height) / sectorSize
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for (i <- matrix.indices) this.matrix(i) = this.matrix(i) combine that.matrix(i)
      this
    }
