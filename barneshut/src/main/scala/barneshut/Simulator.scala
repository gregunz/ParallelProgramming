...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    /*
    b.minX = Math.max(boundaries.minX, body.x)
    b.minY = Math.max(boundaries.minY, body.y)
    b.maxX = Math.min(boundaries.maxX, body.x)
    b.maxY = Math.min(boundaries.maxY, body.y)
    */
