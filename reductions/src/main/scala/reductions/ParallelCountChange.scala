...
object ParallelCountChange {
  ...
  def countChangeAcc(money: Int, coins: List[Int], ways: Int): Int ={
    ???
  }
  ...
  def moneyThreshold(startingMoney: Int): Threshold =
    (money :Int, l: List[Int]) =>  money <= 2*startingMoney/3

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int) =
    (money :Int, l: List[Int]) =>  l.length <= 2*totalCoins/3


  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]) = {
    (money :Int, l: List[Int]) => money*l.length <= startingMoney*allCoins.length/2
  }
}
