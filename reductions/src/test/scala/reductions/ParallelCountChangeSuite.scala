...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("PARALLEL countChange should work when there is only one coin") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, (money :Int, l: List[Int]) => false) == expected,
        s"countChange($money, $coins) should be $expected")
  ...
  test("countChange should work for multi-coins") {
    ...
    check(500, List(1, 2, 5, 10, 20, 50), 3937256)
  }

  test("PARALLEL countChange should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
